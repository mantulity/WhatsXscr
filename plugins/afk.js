...
Xscr.addCommand({on: 'text', fromMe: false, deleteCommand: false}, (async (message, match) => {
            ...
            if (message.jid.includes('-') && (message.mention !== false && message.mention.length !== 0)) {
                message.mention.map(async (jid) => {
                    if (message.client.user.jid.split('@')[0] === jid.split('@')[0]) {
                        ...
                        (AFK.lastseen !== 0 ? '\n*' + Lang.LAST_SEEN + ':* ```' + secondsToHms(Math.round((new Date()).getTime() / 1000) - AFK.lastseen) + Lang.SEEN '```' : ''), MessageType.text, {quoted: message.data});            
                    }
                })
...
Xscr.addCommand({on: 'text', fromMe: true, deleteCommand: false}, (async (message, match) => {
...
Xscr.addCommand({pattern: 'afk ?(.*)', fromMe: true, deleteCommand: false, desc: Lang.AFK_DESC}, (async (message, match) => {     
    if (!AFK.isAfk) {
