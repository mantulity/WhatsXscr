...
const Lang = Language.getString('_xscr');

if (Config.WORKTYPE == 'private') {

    Xscr.addCommand({pattern: 'xscr ?(.*)', fromMe: true, dontAddCommandList: true}, (async (message, match) => {
        var CMD_HELP = '';
        if (match[1] === '') {
            Xscr.commands.map(
                async (command) =>  {
    ...
    Xscr.addCommand({pattern: 'xscr ?(.*)', fromMe: false, dontAddCommandList: true}, (async (message, match) => {
        var CMD_HELP = '';
        if (match[1] === '') {
            Xscr.commands.map(
                async (command) =>  {
